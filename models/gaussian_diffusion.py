...
class GaussianDiffusion:
   ...
   #    model_output, _ = fuse_model(model_output, t, self.pred_z_start_cache, flow)
    ...
    def p_sample(self, model, fuse_model, x, y, t,flow=None,
                 clip_denoised=True, denoised_fn=None, model_kwargs=None, noise_repeat=False):
        """
    ...
    def p_sample_loop(
        ...
        for sample in self.p_sample_loop_progressive(
            ...
            # tt -= 1
            # tmp = self.decode_first_stage(self._scale_input(final, th.tensor([tt],device=final.device)), first_stage_model);tmp = tmp[0].cpu().numpy().transpose(1, 2, 0);tmp = ((tmp * 0.5 + 0.5) * 255).astype(np.uint8);plt.imshow(tmp);plt.show()
        self.pred_z_start_cache = final.detach()
    ...
    def p_sample_loop_progressive(
        ...
        if step_mask is not None:
            pop_number=1 if step_mask<0.3 else 0
    ...
    def training_losses(
        ...
        # terms = {}
        # loss_char=L1_Charbonnier_loss()
        # if self.loss_type == LossType.MSE or self.loss_type == LossType.WEIGHTED_MSE:
        #     with th.no_grad():
        #         predicted_xT_from_gt = model(self._scale_input(z_start, t), t*0, **model_kwargs)
        #         model_output1 = model(self._scale_input(z_t, t), t, **model_kwargs)
        #     model_output = model(self._scale_input(predicted_xT_from_gt.detach(), t), t, **model_kwargs)
        ...
        #     terms["mse"] = mean_flat((target - model_output)**2+(model_output-model_output1)**2)
        #     #terms["mse"] = mean_flat(loss_char(target , model_output))
       ...
       #     model_output, mask = fuse_model(model_output, t, pre_z_start, flow)
            target = {
                ModelMeanType.START_X: z_start,
                ModelMeanType.RESIDUAL: z_y - z_start,
                ModelMeanType.EPSILON: noise,
