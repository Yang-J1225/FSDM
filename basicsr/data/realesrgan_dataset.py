...
@DATASET_REGISTRY.register(suffix='basicsr')
class RealVSRDATA(data.Dataset):
    
    def __init__(self, opt, mode='training',val_mode=False,length=None,):
        super(RealVSRDATA, self).__init__()
        self.opt = opt
        self.file_client = None
        self.io_backend_opt = opt['io_backend']
        
        self.sample_stride = opt['sample_stride']
        self.sample_n_frames = opt['sample_n_frames']
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'training':
            # -------------------- Do augmentation for training: flip, rotation -------------------- #
            img_gt = augment(img_gt, self.opt['use_hflip'], self.opt['use_rot'])
            h, w = img_gt.shape[1:3]
            # while h < crop_pad_size or w < crop_pad_size:
            #     return self[(index+1)%len(self)]
            # if img_gt.shape[1] > crop_pad_size or img_gt.shape[2] > crop_pad_size:
            #     h, w = img_gt.shape[1:3]
            #     # randomly choose top and left coordinates
            #     top = random.randint(0, h - crop_pad_size)
            #     left = random.randint(0, w - crop_pad_size)
            ...
            while h < crop_pad_size or w < crop_pad_size:
                ...
                h, w = img_gt.shape[1:3]
            # crop
            final_image = np.zeros((img_gt.shape[0],self.opt['gt_size'], self.opt['gt_size'],img_gt.shape[3]))
            if img_gt.shape[1] > crop_pad_size or img_gt.shape[2] > crop_pad_size:
                h, w = img_gt.shape[1:3]
                # randomly choose top and left coordinates
                top = random.randint(0, h - crop_pad_size)
            ...
            if self.rescale_gt and crop_pad_size != self.opt['gt_size']:
                #print(img_gt.shape)
                final_image = [cv2.resize(img_gt[i], dsize=(self.opt['gt_size'], self.opt['gt_size']), interpolation=cv2.INTER_AREA) for i in range(len(img_gt))]
                final_image_array = np.stack(final_image, axis=0)
                img_gt = final_image_array
                #print(img_gt.shape)
                #final_image[i] = cv2.resize(img_gt[i], dsize=(self.opt['gt_size'],self.opt['gt_size']), interpolation=cv2.INTER_AREA)
        ...
        for i in range(img_gt.shape[0]):
            ...
            if np.random.uniform() < self.opt['final_sinc_prob']:
                ...
                sinc_kernel = self.pulse_tensor
            kernel_list.append(torch.FloatTensor(kernel))
            kernel2_list.append(torch.FloatTensor(kernel2))
            sinc_kernel_list.append(sinc_kernel)
        sinc_kernel=torch.stack(sinc_kernel_list)
        kernel = torch.stack(kernel_list)
        kernel2 = torch.stack(kernel2_list)
        # BGR to RGB, HWC to CHW, numpy to tensor
        img_gt_list = [img for img in img_gt]
        img_gt_tensor_list = img2tensor(img_gt_list, bgr2rgb=True, float32=True)
        img_gt = torch.stack(img_gt_tensor_list)
