...
class ImageSpliterTh:
    def __init__(self, im, pch_size, stride, sf=1, extra_bs=1, extra_im=None):
        '''
        Input:
    ...
    def __next__(self):
        if self.count_pchs < self.length:
            ...
            for ii, (h_start, w_start) in enumerate(current_starts_list):
                ...
                pch.append(current_pch)

                h_start *= self.sf
                h_end *= self.sf
                w_start *= self.sf
                w_end *= self.sf
                if self.extra_im is not None:
                    current_extra_im = self.extra_im[:, :, h_start:h_end, w_start:w_end]
                    extra_im.append(current_extra_im)
        ...
        return torch.cat(pch, dim=0), index_infos,  None if len(extra_im)==0 else torch.cat(extra_im, dim=0)
