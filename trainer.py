...
class TrainerBase:
    ...
    def save_ckpt(self):
        if self.rank == 0:
            ckpt_path = self.ckpt_dir / '{:d}.pth'.format(self.current_iters)
            torch.save(self.model.state_dict(), ckpt_path)
            if hasattr(self, 'ema_rate'):
                ema_ckpt_path = self.ema_ckpt_dir / 'ema_{:d}.pth'.format(self.current_iters)
...
class TrainerDifIR(TrainerBase):
    ...
    def build_model(self):
        ...
        params = self.configs.model.get('params', dict)
        self.logger.info(f"finetune from {self.configs.model.ckpt_path}")
        self.logger.info(f"flownet from {self.configs.flownet.ckpt_path}")
        self.logger.info(f"lr:{self.configs.train.lr}")
        self.logger.info(f"milestones:{self.configs.train.milestones}")
        model = util_common.get_obj_from_str(self.configs.model.target)(**params)
        ...
        # flownet
        params = self.configs.flownet.get('params', dict)
        flow_net = util_common.get_obj_from_str(self.configs.flownet.target)(**params)
        for params in flow_net.parameters():
            params.requires_grad_(False)
        flow_net.eval()
        self.flow_net = flow_net.cuda()
        self.fuse = None

        # EMA
        if self.rank == 0:
            self.ema_model = deepcopy(self.model).cuda()
    ...
    def prepare_data(self, data, dtype=torch.float32, realesrgan=None, phase='train'):
        ...
        if realesrgan is None:
            realesrgan = self.configs.data.get(phase, dict).type == 'realvsr'
        if realesrgan and phase == 'train':
            ...
            im_gts = data['gt'].cuda()
            b,v_len,c,h,w=im_gts.size()
            self.lq = torch.empty(b,v_len,c,int(h/4),int(w/4))
            self.gt = torch.empty(b,v_len,c,h,w)

            kernel1s = data['kernel1'].cuda()
            kernel2s = data['kernel2'].cuda()
            sinc_kernels = data['sinc_kernel'].cuda()

            ori_h, ori_w = im_gts.size()[3:5]
            ...
            if self.configs.degradation.use_sharp:
                im_gts = self.use_sharpener(im_gts)
            for i in range(im_gts.size()[0]):
            ...
            # blur
                ...
                self.lq[i], self.gt[i], flag_nan = replace_nan_in_batch(im_lq, im_gt)
                if flag_nan:
    ...
    def training_step(self, data):
        ...
        data['lq']=data['lq'].cuda()
        current_batchsize, video_size = data['gt'].shape[:2]
        num_grad_accumulate = video_size
        ...
        latent_resolution = data['gt'].shape[-1] // latent_downsamping_sf
        z0_start = None
        flow = None
        sf = self.configs.diffusion.params.sf
        for jj in range(0, video_size, 1):
            micro_data = {key:value[:, jj,] for key, value in data.items()}
            window_size = self.configs.diffusion.params.window_size
            up_im_lq_tensor = F.interpolate(data['lq'][:, jj], scale_factor=sf, mode='bicubic')
            warp_pre_im_sr_tensor =0
            if jj>0:
                flow_masks = []
                warp_pre_im_sr_tensors = []
                for j in range(1, min(jj+1, window_size+1)):
                    forward_flow = self.flow_net(data['lq'][:,jj]*0.5+0.5,data['lq'][:,jj-j]*0.5+0.5)
                    forward_flow = torch.nn.functional.interpolate(forward_flow * sf,
                                                                   scale_factor=sf).permute(0, 2,
                                                                                                 3, 1)
                    warp_pre_im_sr_tensor = flow_warp(data['gt'][:,jj-j], forward_flow)
                    warp_pre_im_sr_tensors.append(warp_pre_im_sr_tensor)
                    backward_warp_flow = self.flow_net(data['gt'][:,jj-j]*0.5+0.5, warp_pre_im_sr_tensor*0.5+0.5).permute(0, 2,3, 1)
                    flow_consistant_err = torch.norm(forward_flow + backward_warp_flow, p=2, dim=-1).unsqueeze(1)
                    #flow_mask = flow_consistant_err / flow_consistant_err.max()
                    #print(flow_consistant_err.max(),flow_consistant_err.min())
                ...
                if len(flow_masks)==1:
                    ...
                    #up_im_lq_tensor *flow_masks[0]  + (1-flow_masks[0]) * warp_pre_im_sr_tensors[0]
                else:
                    for i in range(len(flow_masks)):
                        ...
                        if i==len(flow_masks)-1:
                            ...
                            weight = 1/(2**(i+1))
                        warp_pre_im_sr_tensor = warp_pre_im_sr_tensor+weight*(up_im_lq_tensor * (1-flow_masks[-(i+1)]) + flow_masks[-(i+1)] * warp_pre_im_sr_tensors[-(i+1)])
                        #warp_pre_im_sr_tensor = warp_pre_im_sr_tensor+weight*(up_im_lq_tensor * flow_masks[-(i+1)] + (1-flow_masks[-(i+1)]) * warp_pre_im_sr_tensors[-(i+1)])
                
            else:
                # lq = util_image.tensor2img(data['lq'][0,0]*0.5+0.5, rgb2bgr=True, min_max=(0.0, 1.0))

                # im_path = "/home/ysj/ResShiftV2/real_lq.png"
                # util_image.imwrite(lq, im_path, chn='bgr', dtype_in='uint8')
                # assert(0)
                warp_pre_im_sr_tensor = up_im_lq_tensor
            last_batch = jj == video_size-1
            noise = torch.randn(
            ...
            #print(micro_data['gt'].device,warp_pre_im_sr_tensor.device)
            compute_losses = functools.partial(
                self.base_diffusion.training_losses,
                self.model,
                micro_data['gt'],
                warp_pre_im_sr_tensor,
                tt,
                ...
                with amp.autocast():
                    if last_batch or self.num_gpus <= 1:
                        ...
                        with self.model.no_sync():
                            losses, z_t, z0_pred, _ = compute_losses()
                    loss = losses["loss"].mean() / num_grad_accumulate
                scaler.scale(loss).backward()
            else:
                if last_batch or self.num_gpus <= 1:
                    losses, z_t, z0_pred, z0_start = compute_losses()
                else:
                    with self.model.no_sync():
                        losses, z_t, z0_pred, z0_start = compute_losses()
                loss = losses["loss"].mean() / num_grad_accumulate
                loss.backward()
            self.log_step_train(losses, tt, z_t, z0_pred, flag=jj==video_size-1)
    ...
    def adjust_lr(self, current_iters=None):
        #self.configs.train.milestones=[2,3,4,25, 500, 1000,1500]
        if len(self.configs.train.milestones) > 0:
            ...
            if current_iters <= linear_steps:
                ...
                for params_group in self.optimizer.param_groups:
                    params_group['lr'] *= 0.5
            #rint(current_iters,self.optimizer.param_groups[0]['lr'])
        else:
            pass

    def log_step_train(self, loss, tt, z_t, z0_pred, flag=False, phase='train'):
        '''
    ...
    def validation(self, phase='val'):
        ...
        if self.rank == 0:
            ...
            indices = [int(self.base_diffusion.num_timesteps * x) for x in [0.1, 0.2, 0.3, 1]]
            batch_size = self.configs.train.batch[1]
            ...
            for ii, data in enumerate(self.dataloaders[phase]):
                video_size = data["gt"].shape[1]

                self.base_diffusion.clear_cache()
                for jj in range(0, video_size, 1):
                    micro_data = {key: value[:, jj, ] for key, value in data.items()}
                    micro_data = self.prepare_data(micro_data, phase='val')
                    if 'gt' in micro_data:
                        im_lq, im_gt = micro_data['lq'], micro_data['gt']
                    else:
                        im_lq = micro_data['lq']
                    num_iters = 0
